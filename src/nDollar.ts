...
let PathDistance = (pts1: Point[], pts2: Point[]): number => {
  let d = 0.0;
...
let PathLength = (points: Point[]): number => {
  let d = 0.0;
...
let Distance = (p1: Point, p2: Point): number => {
  let dx = p2.X - p1.X;
  let dy = p2.Y - p1.Y;
  return Math.sqrt(dx * dx + dy * dy);
}

let CalcStartUnitVector = (points: Point[], index: number): Point => {
...
let AngleBetweenUnitVectors = (v1: Point, v2: Point): number => {
...
let Deg2Rad = (d: number): number => (d * Math.PI / 180.0);

let Resample = (points: Point[], numPoint: number): Point[] => {
  let I = PathLength(points) / (numPoint - 1);
  let D = 0.0;
...
let MakeUnistrokes = (strokes: Point[][], orders: number[][]): Point[][] => {
  ...
  for (let r = 0; r < orders.length; r++) {
    for (let b = 0; b < Math.pow(2, orders[r].length); b++) {
      let unistroke = [];
...
let RotateBy = (points: Point[], radians: number): Point[] => {
  let c = Centroid(points);
...
let ScaleDimTo = (points: Point[], size: number, ratio1D: number) => {
  let B = BoundingBox(points);
  let uniformly = Math.min(B.Width / B.Height, B.Height / B.Width) <= ratio1D;
  let newpoints = [];
  for (let i = 0; i < points.length; i++) {
...
let TranslateTo = (points: Point[], pt: Point): Point[] => {
  let c = Centroid(points);
  let newpoints = [];
  for (let i = 0; i < points.length; i++) {
...
class Recognizer {
  ...
  constructor(public useBoundedRotationInvariance: boolean) {// constructor
    this.Multistrokes = [];
  }
  //
  ...
  Recognize(strokes: Point[][] , requireSameNoOfStrokes: boolean, useProtractor: boolean): Result {
    ...
    let vector = Vectorize(points, this.useBoundedRotationInvariance); // for Protractor
  ...
  AddGesture(name: string, strokes: Point[][]) {
